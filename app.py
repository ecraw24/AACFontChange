...
tmpdirname = tempfile.TemporaryDirectory(suffix=None, prefix=None, dir=os.path.join(os.getcwd(), 'mysite/'))
print('tmpdirname: ', tmpdirname, '; tmpdirname.name: ', tmpdirname.name)
dbFilePath = os.path.join(tmpdirname.name, 'temp.db')
...
def download():
  ...
  if source == 'fontChange':
    #try:
      zipFilePath = [os.path.join(tmpdirname.name, file) for file in os.listdir(tmpdirname.name) if file.endswith('.zip')]
      print('tmpdirpath: ', tmpdirname.name, '; dbFilePath: ', dbFilePath, '; zipFilePathArray: ', zipFilePath)
      newCEPath = file_manipulator.rezipDB(dbFilePath, zipFilePath[0], tmpdirname.name)
